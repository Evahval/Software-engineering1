...
Scenario: Check for available employees to join a new activity
    ...
    And an employee "AJFH" is working on less than 10 activities
 ...
 Scenario: Project manager tries to add an employee with 10 activities, who is not approved for more, to a new activity.
    ...
    Then the system will provide an error-message "the employee is not available for a new activity."
 ...
 Scenario: Project manager tries to add an employee with 10 activities, who is approved to work on 20 activities, to a new activity
    Given an employee is currently working on 10 activities
    And the project manager "HNKJ" of the project "project" "123" wants to check if employee "AJFH" is available
    And the employee has permission
